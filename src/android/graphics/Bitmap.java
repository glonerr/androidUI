...
public final class Bitmap implements Parcelable {
	 ...
	 */
	Bitmap(int nativeBitmap, byte[] buffer, boolean isMutable,
	 ...
	 */
	Bitmap(int nativeBitmap, byte[] buffer, boolean isMutable,
			byte[] ninePatchChunk, int[] layoutBounds, int density) {
		if (nativeBitmap == 0) {
	 ...
	 * 
	 * <p>
	 * The default density is the same density as the current display, unless
	 * the current application does not support different screen densities in
	 * which case it is {@link android.util.DisplayMetrics#DENSITY_DEFAULT}.
	 * Note that compatibility mode is determined by the application that was
	 * initially loaded into a process -- applications that share the same
	 * process should all have the same compatibility, or ensure they explicitly
	 * set the density of their bitmaps appropriately.
	 * </p>
	 * 
	 ...
	 * 
	 * @param bounds
	 *            the array containing the padding values
	 * 
	 * @hide
	 */
	 ...
	 * no other references. The bitmap is marked as "dead", meaning it will
	 * throw an exception if getPixels() or setPixels() is called, and will draw
	 * nothing. This operation cannot be reversed, so it should only be called
	 * if you are sure there are no further uses for the bitmap. This is an
	 * advanced call, and normally need not be called, since the normal GC
	 * process will free up this memory when there are no more references to
	 * this bitmap.
	 */
	public void recycle() {
		if (!mRecycled) {
			if (nativeRecycle(mNativeBitmap)) {
				// return value indicates whether native pixel object was
				// actually recycled.
				// false indicates that it is still in use at the native level
				// and these
				// objects should not be collected now. They will be collected
				// later when the
	 ...
	 * caller). An exception is thrown if the buffer is not large enough to hold
	 * all of the pixels (taking into account the number of bytes per pixel) or
	 * if the Buffer subclass is not one of the support types (ByteBuffer,
	 * ShortBuffer, IntBuffer).
	 * </p>
	 * <p>
	 * The content of the bitmap is copied into the buffer as-is. This means
	 * that if this bitmap stores its pixels pre-multiplied (see
	 * {@link #isPremultiplied()}, the values in the buffer will also be
	 * pre-multiplied.
	 * </p>
	 */
	 ...
	 * overwriting the bitmap's pixels. The data in the buffer is not changed in
	 * any way (unlike setPixels(), which converts from unpremultipled 32bit to
	 ...
	 * @param isMutable
	 *            True if the resulting bitmap should be mutable (i.e. its
	 ...
	 * the specified width and height are the same as the current width and
	 * height of the source btimap, the source bitmap is returned and now new
	 * bitmap is created.
	 * 
	 * @param src
	 *            The source bitmap.
	 ...
	 * @return The new scaled bitmap or the source bitmap if no scaling is
	 *         required.
	 */
	...
	/**
	 * Returns an immutable bitmap from the source bitmap. The new bitmap may be
	 * the same object as source, or a copy may have been made. It is
	 ...
	 * @param height
	 *            The number of rows
	 * @return A copy of a subset of the source bitmap or the source bitmap
	 *         itself.
	 */
	...
	/**
	 * Returns an immutable bitmap from subset of the source bitmap, transformed
	 * by the optional matrix. The new bitmap may be the same object as source,
	 * or a copy may have been made. It is initialized with the same density as
	 * the original bitmap.
	 * 
	 * If the source bitmap is immutable and the requested subset is the same as
	 * the source bitmap itself, then the source bitmap is returned and no new
	 * bitmap is created.
	 * 
	 * @param source
	 *            The bitmap we are subsetting
	 ...
	 *             if the width or height are <= 0
	 */
	private static Bitmap createBitmap(DisplayMetrics display, int width,
			int height, Config config, boolean hasAlpha) {
		if (width <= 0 || height <= 0) {
	 ...
	 * 
	 * @param colors
	 *            Array of {@link Color} used to initialize the pixels.
	 * @param offset
	 ...
	 * @throws IllegalArgumentException
	 *             if the width or height are <= 0, or if the color array's
	 *             length is less than the number of pixels.
	 */
	 ...
	 * 
	 * @param colors
	 *            Array of {@link Color} used to initialize the pixels. This
	...
	public boolean compress(CompressFormat format, int quality,
		...
		return nativeCompress(mNativeBitmap, format.nativeInt, quality, stream,
	...
	/**
	 * <p>
	 * Indicates whether pixels stored in this bitmaps are stored
	 * pre-multiplied. When a pixel is pre-multiplied, the RGB components have
	 * been multiplied by the alpha component. For instance, if the original
	 * color is a 50% translucent red <code>(128, 255, 0, 0)</code>, the
	 * pre-multiplied form is <code>(128, 128, 0, 0)</code>.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * This method only returns true if {@link #hasAlpha()} returns true. A
	 * bitmap with no alpha channel can be used both as a pre-multiplied and as
	 * a non pre-multiplied bitmap.
	 * </p>
	 * 
	...
	static public int scaleFromDensity(int size, int sdensity, int tdensity) {
		if (sdensity == DENSITY_NONE || tdensity == DENSITY_NONE
				|| sdensity == tdensity) {
			return size;
		}
	 ...
	 * pixels may contain non-opaque alpha values. For some configs, this is
	 * always false (e.g. RGB_565), since they do not support per-pixel alpha.
	 * However, for configs that do, the bitmap may be flagged to be known that
	 * all of its pixels are opaque. In this case hasAlpha() will also return
	 * false. If a config such as ARGB_8888 is not so flagged, it will return
	 * true by default.
	 */
	 ...
	 * some of the pixels may contain non-opaque alpha values (true). Note, for
	 * some configs (e.g. RGB_565) this call is ignored, since it does not
	 * support per-pixel alpha values.
	 * 
	 ...
	 * 
	 * If you know that you are going to draw this bitmap at less than 50% of
	 * its original size, you may be able to obtain a higher quality
	 * 
	 * This property is only a suggestion that can be ignored by the renderer.
	 * It is not guaranteed to have any effect.
	 * 
	 * @return true if the renderer should attempt to use mipmaps, false
	 *         otherwise
	 * 
	 ...
	 * indicating that it should attempt to use mipmaps when this bitmap is
	 * drawn scaled down.
	 * 
	 * If you know that you are going to draw this bitmap at less than 50% of
	 * its original size, you may be able to obtain a higher quality by turning
	 * this property on.
	 * 
	 * Note that if the renderer respects this hint it might have to allocate
	 * extra memory to hold the mipmap levels for this bitmap.
	 * 
	 * This property is only a suggestion that can be ignored by the renderer.
	 * It is not guaranteed to have any effect.
	 * 
	...
	/**
	 * Returns in pixels[] a copy of the data in the bitmap. Each value is a
	 * packed int representing a {@link Color}. The stride parameter allows the
	 * caller to allow for gaps in the returned pixels array between rows. For
	 * normal packed results, just pass width for the stride value. The returned
	 * colors are non-premultiplied ARGB values.
	 * 
	 * @param pixels
	 ...
	 * @param stride
	 *            The number of entries in pixels[] to skip between rows (must
	...
	public void getPixels(int[] pixels, int offset, int stride, int x, int y,
		...
		nativeGetPixels(mNativeBitmap, pixels, offset, stride, x, y, width,
				height);
	}

	/**
	 * Shared code to check for illegal arguments passed to getPixel() or
	 * setPixel()
	 * 
	 * @param x
	 ...
	 * mutable) at the x,y coordinate. The color must be a non-premultiplied
	 * ARGB value.
	 * </p>
	 * 
	 * @param x
	 ...
	 *            bitmap, but it can be larger (or negative).
	 * @param x
	...
	public void setPixels(int[] pixels, int offset, int stride, int x, int y,
		...
		checkPixelsAccess(x, y, width, height, offset, stride, pixels);
		nativeSetPixels(mNativeBitmap, pixels, offset, stride, x, y, width,
				height);
	}
	...
	/**
	 * Write the bitmap and its pixels to the parcel. The bitmap can be rebuilt
	...
	/**
	 * Returns a new bitmap that captures the alpha values of the original. This
	 * may be drawn with Canvas.drawBitmap(), where the color(s) will be taken
	 * from the paint that is passed to the draw call.
	 * 
	 ...
	 * These values may be affected by the optional Paint parameter, which can
	 * contain its own alpha, and may also contain a MaskFilter which could
	 * change the actual dimensions of the resulting bitmap (e.g. a blur
	 * maskfilter might enlarge the resulting bitmap). If offsetXY is not null,
	 * it returns the amount to offset the returned bitmap so that it will
	 * logically align with the original. For example, if the paint contains a
	 * blur of radius 2, then offsetXY[] would contains -2, -2, so that drawing
	 * the alpha bitmap offset by (-2, -2) and then drawing the original would
	 * result in the blur visually aligning with the original.
	 * 
	 * <p>
	 ...
	 * @param offsetXY
	 *            Optional array that returns the X (index 0) and Y (index 1)
	...
	public boolean sameAs(Bitmap other) {
		return this == other
				|| (other != null && nativeSameAs(mNativeBitmap,
						other.mNativeBitmap));
	}

	/**
	 * Rebuilds any caches associated with the bitmap that are used for drawing
	 * it. In the case of purgeable bitmaps, this call will attempt to ensure
	 * that the pixels have been decoded. If this is called on more than one
	 * bitmap in sequence, the priority is given in LRU order (i.e. the last
	 * bitmap called will be given highest priority).
	 * 
