 ...
 * <p>
 * For more information about how to use Canvas, read the <a href="{@docRoot}
...
public class Canvas {
	 ...
	 * 
	 * @return True if drawing operations are hardware accelerated, false
	 *         otherwise.
	 */
	...
	public void setBitmap(Bitmap bitmap) {
		if (isHardwareAccelerated()) {
			throw new RuntimeException(
					"Can't set a bitmap device on a GL canvas");
		}
	...
	public int saveLayer(RectF bounds, Paint paint, int saveFlags) {
		return native_saveLayer(mNativeCanvas, bounds,
				paint != null ? paint.mNativePaint : 0, saveFlags);
	}
	...
	public boolean clipRect(RectF rect, Region.Op op) {
		return native_clipRect(mNativeCanvas, rect.left, rect.top, rect.right,
				rect.bottom, op.nativeInt);
	}
	...
	public void drawArc(RectF oval, float startAngle, float sweepAngle,
		...
		native_drawArc(mNativeCanvas, oval, startAngle, sweepAngle, useCenter,
				paint.mNativePaint);
	}
	 ...
	 * @param chunks
	 *            The patches information (matches the native struct
	 *            Res_png_9patch)
	 * @param dst
	...
	public void drawBitmap(int[] colors, int offset, int stride, float x,
		...
		native_drawBitmap(mNativeCanvas, colors, offset, stride, x, y, width,
				height, hasAlpha, paint != null ? paint.mNativePaint : 0);
	}
	...
	public void drawVertices(VertexMode mode, int vertexCount, float[] verts,
		...
		nativeDrawVertices(mNativeCanvas, mode.nativeInt, vertexCount, verts,
				vertOffset, texs, texOffset, colors, colorOffset, indices,
				indexOffset, indexCount, paint.mNativePaint);
	}
	 ...
	 * @param contextIndex
	 *            the start of the context for shaping. Must be no greater than
	 *            index.
	 * @param contextCount
	 ...
	 * @param dir
	 *            the run direction, either {@link #DIRECTION_LTR} or
	 *            {@link #DIRECTION_RTL}.
	 * @param paint
	 *            the paint
	 * @hide
	...
	public void drawTextRun(char[] text, int index, int count,
		...
		native_drawTextRun(mNativeCanvas, text, index, count, contextIndex,
	 ...
	 *            the text to render
	 * @param start
	 *            the start of the text to render. Data before this position can
	 *            be used for shaping context.
	 * @param end
	 *            the end of the text to render. Data at or after this position
	 ...
	 *            the y position at which to draw the text
	 * @param dir
	 *            the run direction, either 0 for LTR or 1 for RTL.
	 * @param paint
	 *            the paint
	 * @hide
	 */
	...
	public void drawTextOnPath(char[] text, int index, int count, Path path,
		...
		native_drawTextOnPath(mNativeCanvas, text, index, count, path.ni(),
				hOffset, vOffset, paint.mBidiFlags, paint.mNativePaint);
	}
	...
	public void drawTextOnPath(String text, Path path, float hOffset,
		...
		if (text.length() > 0) {
			native_drawTextOnPath(mNativeCanvas, text, path.ni(), hOffset,
					vOffset, paint.mBidiFlags, paint.mNativePaint);
		}
	}
