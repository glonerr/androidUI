...
public class Paint {
   ...
   /**
    ...
    public Paint(int flags) {
        ...
        mTypeface = Typeface.create((String)null, 0);
    ...
    public void setTextLocale(Locale locale) {
        ...
        mLocale = locale;
        if(mTypeface != null)
        	mTypeface.mFontData.setLocale(locale.toString());
//        native_setTextLocale(mNativePaint, locale.toString());
    ...
    public float getTextSize(){
    	return mTypeface.mFontData.height;
    }
    ...
    public void setTextSize(float textSize){
    	mTypeface.mFontData.height = textSize; 
    }
    ...
    public int getFontMetricsInt(FontMetricsInt fmi){
    	if(fmi != null){
    		GC gc = new GC(sImage);
        	gc.setFont(new Font(null, mTypeface.mFontData));
        	org.eclipse.swt.graphics.FontMetrics metrics = gc.getFontMetrics(); 
    		fmi.ascent = metrics.getAscent();
    		fmi.descent = metrics.getDescent();
    		fmi.top = 0;
    		fmi.bottom = metrics.getHeight();
    		fmi.leading = metrics.getLeading();
    		gc.dispose();
    		return fmi.bottom;
    	}else 
    		return 0;
    ...
    private float native_measureText(String text, int start, int end){
    	return native_measureText(text.toCharArray(), start, end-start);
    }
    ...
    private float native_measureText(String text){
    	return native_measureText(text, 0, text.length()-1);
    }
    ...
    public float getTextRunAdvances(char[] chars, int index, int count,
        ...
        if (!mHasCompatScaling) {
        	return getStringWidth(chars, index, count);
...
//                    contextIndex, contextCount, flags, advances, advancesIndex, reserved);
    ...
    public float getTextRunAdvances(String text, int start, int end, int contextStart,
        ...
        if (!mHasCompatScaling) {
        	return getStringWidth(text, start, end);
...
//                    contextStart, contextEnd, flags, advances, advancesIndex, reserved);
    ...
    private static int sNativePaint;
    private int mFlags;
    
    boolean mAntialias;
    int mColor;
    
    private int getStringWidth(String string,int start,int end) {
        GC gc = new GC(sImage);
        if(mTypeface != null)
        	gc.setFont(new Font(null, mTypeface.mFontData));
        int width = gc.stringExtent(string.substring(start, end)).x;
        gc.dispose();
        return width;
    }
    
    private int getStringWidth(char[] cs,int index,int count) {
        GC gc = new GC(sImage);
        if(mTypeface != null)
        	gc.setFont(new Font(null, mTypeface.mFontData));
        int width = gc.stringExtent(new String(cs, index, count)).x;
        gc.dispose();
        return width;
    }
    
    static Image sImage = new Image(null, 1, 1);
}
