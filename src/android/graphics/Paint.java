...
import com.lonerr.bridge.graphics.PaintBridge;
...
public class Paint {
	...
	static final Style[] sStyleArray = { Style.FILL, Style.STROKE,
	...
	/**
	 * Option for getTextRunCursor to compute the valid cursor after offset or
	 * the limit of the context, whichever is less.
	 * 
	 * @hide
	 */
	...
	/**
	 * Option for getTextRunCursor to compute the valid cursor at or after the
	...
	/**
	 * Option for getTextRunCursor to compute the valid cursor before offset or
	 * the start of the context, whichever is greater.
	 * 
	 * @hide
	 */
	...
	/**
	 * Option for getTextRunCursor to return offset if the cursor at offset is
	 * valid, or -1 if it isn't.
	 * 
	 * @hide
	 */
	...
	public enum Style {
		 ...
		 * paint. This mode can give unexpected results if the geometry is
		 * oriented counter-clockwise. This restriction does not apply to either
		 * FILL or STROKE.
		 */
		FILL_AND_STROKE(2);
	 ...
	 */
	public int getFlags() {
		return PaintBridge.getFlags(mNativePaint);
	}
	...
	public void setFlags(int flags) {
		PaintBridge.setFlags(mNativePaint, flags);
	}
	...
	public int getHinting() {
		return PaintBridge.getHinting(mNativePaint);
	}
	...
	public void setHinting(int mode) {
		PaintBridge.setHinting(mNativePaint, mode);
	}
	...
	public void setAntiAlias(boolean aa) {
		PaintBridge.setAntiAlias(mNativePaint, aa);
	}
	...
	public void setDither(boolean dither) {
		PaintBridge.setDither(mNativePaint, dither);
	}
	...
	public void setLinearText(boolean linearText) {
		PaintBridge.setLinearText(mNativePaint, linearText);
	}
	...
	public void setSubpixelText(boolean subpixelText) {
		PaintBridge.setSubpixelText(mNativePaint, subpixelText);
	}
	...
	public void setUnderlineText(boolean underlineText) {
		PaintBridge.setUnderlineText(mNativePaint, underlineText);
	}
	...
	public void setStrikeThruText(boolean strikeThruText) {
		PaintBridge.setStrikeThruText(mNativePaint, strikeThruText);
	}
	...
	public void setFakeBoldText(boolean fakeBoldText) {
		PaintBridge.setFakeBoldText(mNativePaint, fakeBoldText);
	}
	...
	public void setFilterBitmap(boolean filter) {
		PaintBridge.setFilterBitmap(mNativePaint, filter);
	}
	 ...
	 */
	public int getColor() {
		return PaintBridge.getColor(mNativePaint);
	}
	...
	public void setColor(int color) {
		PaintBridge.setColor(mNativePaint, color);
	}
	 ...
	 */
	public int getAlpha() {
		return PaintBridge.getAlpha(mNativePaint);
	}
	...
	public void setAlpha(int a) {
		PaintBridge.setAlpha(mNativePaint, a);
	}
	...
	public float getStrokeWidth() {
		return PaintBridge.getStrokeWidth(mNativePaint);
	}
	...
	public void setStrokeWidth(float width) {
		PaintBridge.setStrokeWidth(mNativePaint, width);
	}
	...
	public float getStrokeMiter() {
		return PaintBridge.getStrokeMiter(mNativePaint);
	}
	...
	public void setStrokeMiter(float miter) {
		PaintBridge.setStrokeMiter(mNativePaint, miter);
	}
	...
	/**
	 * This draws a shadow layer below the main layer, with the specified offset
	 * and color, and blur radius. If radius is 0, then the shadow layer is
	 * removed.
	 */
	...
	private void nSetShadowLayer(float radius, float dx, float dy, int color) {
		PaintBridge.nSetShadowLayer(mNativePaint, radius, dx, dy, color);
	}
	 ...
	 * 
	 * For example, if the locale is {@link Locale#CHINESE} or
	 * {@link Locale#CHINA}, then the text renderer will prefer to draw text
	 * using a Chinese font. Likewise, if the locale is {@link Locale#JAPANESE}
	 * or {@link Locale#JAPAN}, then the text renderer will prefer to draw text
	 * using a Japanese font.
	 * 
	 * This distinction is important because Chinese and Japanese text both use
	 * many of the same Unicode code points but their appearance is subtly
	 * different for each language.
	 * 
	 * By default, the text locale is initialized to the system locale (as
	 * returned by {@link Locale#getDefault}). This assumes that the text to be
	 * rendered will most likely be in the user's preferred language.
	 * 
	 * If the actual language of the text is known, then it can be provided to
	 * the text renderer using this method. The text renderer may attempt to
	 * guess the language script based on the contents of the text to be drawn
	 * independent of the text locale here. Specifying the text locale just
	 * helps it do a better job in certain ambiguous cases
	 * 
	 * @param locale
	...
	public float getTextSize() {
		return PaintBridge.getTextSize(mNativePaint);
	}
	...
	public void setTextSize(float textSize) {
		PaintBridge.setTextSize(mNativePaint, textSize);
	}
	...
	public float getTextScaleX() {
		return PaintBridge.getTextScaleX(mNativePaint);
	}
	...
	public void setTextScaleX(float scaleX) {
		PaintBridge.setTextScaleX(mNativePaint, scaleX);
	}
	...
	public float getTextSkewX() {
		return PaintBridge.getTextSkewX(mNativePaint);
	}
	...
	public void setTextSkewX(float skewX) {
		PaintBridge.setTextSkewX(mNativePaint, skewX);
	}
	 ...
	 */
	public float ascent() {
		return PaintBridge.ascent(mNativePaint);
	}
	...
	public float descent() {
		return PaintBridge.descent(mNativePaint);
	}
	...
	public float getFontMetrics(FontMetrics metrics) {
		return PaintBridge.getFontMetrics(mNativePaint, metrics);
	}
	...
	public int getFontMetricsInt(FontMetricsInt fmi) {
		return PaintBridge.getFontMetricsInt(mNativePaint, fmi);
	}
	...
	private float native_measureText(char[] text, int index, int count) {
		return PaintBridge.native_measureText(mNativePaint, text, index, count);
	}
	...
	private float native_measureText(String text, int start, int end) {
		return PaintBridge.native_measureText(mNativePaint, text, start, end);
	}
	...
	private float native_measureText(String text) {
		return PaintBridge.native_measureText(mNativePaint, text);
	}
	 ...
	 *            is negative, then the characters are measured in reverse
	 *            order.
	 * @param maxWidth
	...
	public int breakText(char[] text, int index, int count, float maxWidth,
		...
		final float oldSize = getTextSize();
		setTextSize(oldSize * mCompatScaling);
		int res = native_breakText(text, index, count, maxWidth
				* mCompatScaling, measuredWidth);
		setTextSize(oldSize);
	...
	private int native_breakText(char[] text, int index, int count,
		...
		return PaintBridge.native_breakText(mNativePaint, text, index, count,
	...
	public int breakText(String text, boolean measureForwards, float maxWidth,
		...
		if (!mHasCompatScaling) {
			return native_breakText(text, measureForwards, maxWidth,
					measuredWidth);
		}

		final float oldSize = getTextSize();
		setTextSize(oldSize * mCompatScaling);
		int res = native_breakText(text, measureForwards, maxWidth
				* mCompatScaling, measuredWidth);
		setTextSize(oldSize);
		if (measuredWidth != null)
			measuredWidth[0] *= mInvCompatScaling;
	...
	private int native_breakText(String text, boolean measureForwards,
			float maxWidth, float[] measuredWidth) {
		return PaintBridge.native_breakText(mNativePaint, text,
	...
	public int getTextWidths(char[] text, int index, int count, float[] widths) {
		...
		if (!mHasCompatScaling) {
			return native_getTextWidths(mNativePaint, text, index, count,
					widths);
		}
	 ...
	 * @param glyphs
	 *            array to receive the glyph Ids of the characters. Must be at
	 *            least a large as the text.
	 ...
	 */
	public int getTextGlyphs(String text, int start, int end, int contextStart,
		...
		if ((start | end | contextStart | contextEnd | (end - start)
				...
				| (text.length() - end) | (text.length() - contextEnd)) < 0) {
			throw new IndexOutOfBoundsException();
		}
		if (end - start > glyphs.length) {
			throw new ArrayIndexOutOfBoundsException();
		}
		return native_getTextGlyphs(mNativePaint, text, start, end,
				contextStart, contextEnd, flags, glyphs);
	}
	...
	public float getTextRunAdvances(char[] chars, int index, int count,
			...
			int advancesIndex) {
		return getTextRunAdvances(chars, index, count, contextIndex,
				contextCount, flags, advances, advancesIndex, 0 /* use Harfbuzz */);
	}
	...
	public float getTextRunAdvances(char[] chars, int index, int count,
		...
		if ((index | count | contextIndex | contextCount | advancesIndex
				...
				| (chars.length - (contextIndex + contextCount)) | (advances == null ? 0
		...
		final float oldSize = getTextSize();
		setTextSize(oldSize * mCompatScaling);
		float res = native_getTextRunAdvances(mNativePaint, chars, index,
				count, contextIndex, contextCount, flags, advances,
	...
	public float getTextRunAdvances(CharSequence text, int start, int end,
			...
			int advancesIndex) {
		return getTextRunAdvances(text, start, end, contextStart, contextEnd,
				flags, advances, advancesIndex, 0 /* use Harfbuzz */);
	}
	...
	public float getTextRunAdvances(CharSequence text, int start, int end,
		...
		if ((start | end | contextStart | contextEnd | advancesIndex
				| (end - start) | (start - contextStart) | (contextEnd - end)
				| (text.length() - contextEnd) | (advances == null ? 0
					: (advances.length - advancesIndex - (end - start)))) < 0) {
			throw new IndexOutOfBoundsException();
		}

		if (text instanceof String) {
			return getTextRunAdvances((String) text, start, end, contextStart,
		...
		float result = getTextRunAdvances(buf, start - contextStart, len, 0,
				contextLen, flags, advances, advancesIndex, reserved);
		TemporaryBuffer.recycle(buf);
		return result;
	}

	/**
	 * Returns the total advance width for the characters in the run between
	 * start and end, and if advances is not null, the advance assigned to each
	 * of these characters (java chars).
	 * 
	 * <p>
	 * The trailing surrogate in a valid surrogate pair is assigned an advance
	 * of 0. Thus the number of returned advances is always equal to count, not
	 * to the number of unicode codepoints represented by the run.
	 * 
	 * <p>
	 * In the case of conjuncts or combining marks, the total advance is
	 * assigned to the first logical character, and the following characters are
	 * assigned an advance of 0.
	 * 
	 * <p>
	 * This generates the sum of the advances of glyphs for characters in a
	 * reordered cluster as the width of the first logical character in the
	 * cluster, and 0 for the widths of all other characters in the cluster. In
	 * effect, such clusters are treated like conjuncts.
	 * 
	 * <p>
	 * The shaping bounds limit the amount of context available outside start
	 * and end that can be used for shaping analysis. These bounds typically
	 * reflect changes in bidi level or font metrics across which shaping does
	 * not occur.
	 * 
	 * @param text
	 *            the text to measure. Cannot be null.
	 ...
	 * @param end
	 *            the index past the last character to measure
	 * @param contextStart
	 *            the index of the first character to use for shaping context,
	 *            must be <= start
	 * @param contextEnd
	 ...
	 *            {@link #DIRECTION_LTR} or {@link #DIRECTION_RTL}
	 * @param advances
	 *            array to receive the advances, must have room for all
	 *            advances, can be null if only total advance is needed
	 * @param advancesIndex
	 *            the position in advances at which to put the advance
	 *            corresponding to the character at start
	 * @return the total advance
	 * 
	 * @hide
	 */
	public float getTextRunAdvances(String text, int start, int end,
			int contextStart, int contextEnd, int flags, float[] advances,
			int advancesIndex) {
		return getTextRunAdvances(text, start, end, contextStart, contextEnd,
				flags, advances, advancesIndex, 0 /* use Harfbuzz */);
	...
	public float getTextRunAdvances(String text, int start, int end,
		...
		final float oldSize = getTextSize();
		setTextSize(oldSize * mCompatScaling);
		float totalAdvance = native_getTextRunAdvances(mNativePaint, text,
				start, end, contextStart, contextEnd, flags, advances,
				advancesIndex, reserved);
		setTextSize(oldSize);
		...
		return totalAdvance * mInvCompatScaling; // assume errors are
													// insignificant
	}
	 ...
	 * 
	 * <p>
	 * ContextStart and offset are relative to the start of text. The context is
	 * the shaping context for cursor movement, generally the bounds of the
	 * metric span enclosing the cursor in the direction of movement.
	 * 
	 * <p>
	 * If cursorOpt is {@link #CURSOR_AT} and the offset is not a valid cursor
	 * position, this returns -1. Otherwise this will never return a value
	 * before contextStart or after contextStart + contextLength.
	 * 
	 * @param text
	 *            the text
	 * @param contextStart
	 ...
	 * @param flags
	 *            either {@link #DIRECTION_RTL} or {@link #DIRECTION_LTR}
	 * @param offset
	 *            the cursor position to move from
	 * @param cursorOpt
	 *            how to move the cursor, one of {@link #CURSOR_AFTER},
	 *            {@link #CURSOR_AT_OR_AFTER}, {@link #CURSOR_BEFORE},
	 *            {@link #CURSOR_AT_OR_BEFORE}, or {@link #CURSOR_AT}
	 * @return the offset of the next position, or -1
	 * @hide
	 */
	public int getTextRunCursor(char[] text, int contextStart,
			int contextLength, int flags, int offset, int cursorOpt) {
		...
		return native_getTextRunCursor(mNativePaint, text, contextStart,
	 ...
	 * 
	 * <p>
	 * ContextStart, contextEnd, and offset are relative to the start of text.
	 * The context is the shaping context for cursor movement, generally the
	 * bounds of the metric span enclosing the cursor in the direction of
	 * movement.
	 * 
	 * <p>
	 * If cursorOpt is {@link #CURSOR_AT} and the offset is not a valid cursor
	 * position, this returns -1. Otherwise this will never return a value
	 * before contextStart or after contextEnd.
	 * 
	 * @param text
	 *            the text
	 * @param contextStart
	...
	public int getTextRunCursor(CharSequence text, int contextStart,
		...
		if (text instanceof GraphicsOperations) {
			return ((GraphicsOperations) text).getTextRunCursor(contextStart,
					contextEnd, flags, offset, cursorOpt, this);
		}

		int contextLen = contextEnd - contextStart;
		char[] buf = TemporaryBuffer.obtain(contextLen);
		TextUtils.getChars(text, contextStart, contextEnd, buf, 0);
		int result = getTextRunCursor(buf, 0, contextLen, flags, offset
				- contextStart, cursorOpt);
		TemporaryBuffer.recycle(buf);
		return result;
	}
	...
	private static int native_init() {
		return PaintBridge.native_init();
	}

	private static int native_initWithPaint(int paint) {
		return PaintBridge.native_initWithPaint(paint);
	}

	private static void native_reset(int native_object) {
		PaintBridge.native_reset(native_object);
	}

	private static void native_set(int native_dst, int native_src) {
		PaintBridge.native_set(native_dst, native_src);
	}

	private static int native_getStyle(int native_object) {
		return PaintBridge.native_getStyle(native_object);
	}

	private static void native_setStyle(int native_object, int style) {
		PaintBridge.native_setStyle(native_object, style);
	}

	private static int native_getStrokeCap(int native_object) {
		return PaintBridge.native_getStrokeCap(native_object);
	}

	private static void native_setStrokeCap(int native_object, int cap) {
		PaintBridge.native_setStrokeCap(native_object, cap);
	}

	private static int native_getStrokeJoin(int native_object) {
		return PaintBridge.native_getStrokeJoin(native_object);
	}

	private static void native_setStrokeJoin(int native_object, int join) {
		PaintBridge.native_setStrokeJoin(native_object, join);
	}

	private static boolean native_getFillPath(int native_object, int src,
			int dst) {
		return PaintBridge.native_getFillPath(native_object, src, dst);
	}

	private static int native_setShader(int native_object, int shader) {
		return PaintBridge.native_setShader(native_object, shader);
	}

	private static int native_setColorFilter(int native_object, int filter) {
		return PaintBridge.native_setColorFilter(native_object, filter);
	}

	private static int native_setXfermode(int native_object, int xfermode) {
		return PaintBridge.native_setXfermode(native_object, xfermode);
	}

	private static int native_setPathEffect(int native_object, int effect) {
		return PaintBridge.native_setPathEffect(native_object, effect);
	}

	private static int native_setMaskFilter(int native_object, int maskfilter) {
		return PaintBridge.native_setMaskFilter(native_object, maskfilter);
	}

	private static int native_setTypeface(int native_object, int typeface) {
		return PaintBridge.native_setTypeface(native_object, typeface);
	}

	private static int native_setRasterizer(int native_object, int rasterizer) {
		return PaintBridge.native_setRasterizer(native_object, rasterizer);
	}

	private static int native_getTextAlign(int native_object) {
		return PaintBridge.native_getTextAlign(native_object);
	}

	private static void native_setTextAlign(int native_object, int align) {
		PaintBridge.native_setTextAlign(native_object, align);
	}

	private static void native_setTextLocale(int native_object, String locale) {
		PaintBridge.native_setTextLocale(native_object, locale);
	}

	private static int native_getTextWidths(int native_object, char[] text,
			int index, int count, float[] widths) {
		return PaintBridge.native_getTextWidths(native_object, text, index,
				count, widths);
	}

	private static int native_getTextWidths(int native_object, String text,
			int start, int end, float[] widths) {
		return PaintBridge.native_getTextWidths(native_object, text, start,
				end, widths);
	}

	private static int native_getTextGlyphs(int native_object, String text,
			int start, int end, int contextStart, int contextEnd, int flags,
			char[] glyphs) {
		return PaintBridge.native_getTextGlyphs(native_object, text, start,
				end, contextStart, contextEnd, flags, glyphs);
	}

	private static float native_getTextRunAdvances(int native_object,
			char[] text, int index, int count, int contextIndex,
			int contextCount, int flags, float[] advances, int advancesIndex,
			int reserved) {
		return PaintBridge.native_getTextRunAdvances(native_object, text,
				index, count, contextIndex, contextCount, flags, advances,
				advancesIndex, reserved);
	}
	...
	private int native_getTextRunCursor(int native_object, char[] text,
			int contextStart, int contextLength, int flags, int offset,
			int cursorOpt) {
		return PaintBridge.native_getTextRunCursor(native_object, text,
				contextStart, contextLength, flags, offset, cursorOpt);
	}

	private int native_getTextRunCursor(int native_object, String text,
			int contextStart, int contextEnd, int flags, int offset,
			int cursorOpt) {
		return PaintBridge.native_getTextRunCursor(native_object, text,
	...
	private static void native_getTextPath(int native_object, int bidiFlags,
		...
		PaintBridge.native_getTextPath(native_object, bidiFlags, text, index,
				count, x, y, path);
	}

	private static void native_getTextPath(int native_object, int bidiFlags,
			String text, int start, int end, float x, float y, int path) {
		PaintBridge.native_getTextPath(native_object, bidiFlags, text, start,
				end, x, y, path);
	}

	private static void nativeGetStringBounds(int nativePaint, String text,
			int start, int end, Rect bounds) {
		PaintBridge
				.nativeGetStringBounds(nativePaint, text, start, end, bounds);
	}

	private static void nativeGetCharArrayBounds(int nativePaint, char[] text,
			int index, int count, Rect bounds) {
		PaintBridge.nativeGetCharArrayBounds(nativePaint, text, index, count,
				bounds);
	}

	private static void finalizer(int nativePaint) {
		PaintBridge.finalizer(nativePaint);
	}
}
