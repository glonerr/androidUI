...
import com.lonerr.bridge.graphics.BitmapFactoryBridge;
...
public class BitmapFactory {
	...
	public static class Options {
		 ...
		 * same size as the source content and in jpeg or png format (whether as
		 * a resource or as a stream). The
		 ...
		 * outlined above and failure situations that can occur. Checking
		 * whether the return value matches the value of the inBitmap set in the
		 * Options structure is a way to see if the bitmap was reused, but in
		 * all cases you should use the returned Bitmap to make sure that you
		 * are using the bitmap that was used as the decode destination.
		 * </p>
		 */
		 ...
		 * an immutable one. This can be used for instance to programmatically
		 * apply effects to a Bitmap loaded through BitmapFactory.
		 */
		...
		/**
		 * The pixel density to use for the bitmap. This will always result in
		 * the returned bitmap having a density set for it (see
		 * {@link Bitmap#setDensity(int) Bitmap.setDensity(int)}). In addition,
		 * if {@link #inScaled} is set (which it is by default} and this density
		 * does not match {@link #inTargetDensity}, then the bitmap will be
		 * scaled to the target density before being returned.
		 * 
		 * <p>
		 * If this is 0, {@link BitmapFactory#decodeResource(Resources, int)},
		 * {@link BitmapFactory#decodeResource(Resources, int, android.graphics.BitmapFactory.Options)}
		 * , and {@link BitmapFactory#decodeResourceStream} will fill in the
		 * density associated with the resource. The other functions will leave
		 * it as-is and no density will be applied.
		 * 
		 * @see #inTargetDensity
		 * @see #inScreenDensity
		 * @see #inScaled
		 ...
		 * {@link BitmapFactory#decodeResource(Resources, int, android.graphics.BitmapFactory.Options)}
		 * , and {@link BitmapFactory#decodeResourceStream} will fill in the
		 * density associated the Resources object's DisplayMetrics. The other
		 ...
		 * {@link #inTargetDensity} is actually the density the application sees
		 * rather than the real screen density.
		 * 
		 * <p>
		 * By setting this, you allow the loading code to avoid scaling a bitmap
		 * that is currently in the screen density up/down to the compatibility
		 * density. Instead, if {@link #inDensity} is the same as
		 * {@link #inScreenDensity}, the bitmap will be left as-is. Anything
		 * using the resulting bitmap must also used
		 * {@link Bitmap#getScaledWidth(int) Bitmap.getScaledWidth} and
		 * {@link Bitmap#getScaledHeight Bitmap.getScaledHeight} to account for
		 * any different between the bitmap's density and the target's density.
		 * 
		 * <p>
		 ...
		 * {@link #inTargetDensity} are not 0, the bitmap will be scaled to
		 * match {@link #inTargetDensity} when loaded, rather than relying on
		 * the graphics system scaling it each time it is drawn to a Canvas.
		 * 
		 * <p>
		 * This flag is turned on by default and should be turned off if you
		 * need a non-scaled version of the bitmap. Nine-patch bitmaps ignore
		 * this flag and are always scaled.
		 */
		 ...
		 * encoded data, either by sharing a reference to the input or by making
		 * a copy of it. This distinction is controlled by inInputShareable. If
		 * this is true, then the bitmap may keep a shallow reference to the
		 * input. If this is false, then the bitmap will explicitly make a copy
		 * of the input data, and keep that. Even if sharing is allowed, the
		 * implementation may still decide to make a deep copy of the input
		 * data.
		 */
	...
	public static Bitmap decodeStream(InputStream is, Rect outPadding,
		...
		if (is instanceof AssetManager.AssetInputStream) {
			final int asset = ((AssetManager.AssetInputStream) is)
					.getAssetInt();
	...
	private static Bitmap finishDecode(Bitmap bm, Rect outPadding, Options opts) {
		...
		if (opts.inScaled || isNinePatch) {
			...
			if (scale != 1.0f) {
				...
				bm = Bitmap.createScaledBitmap(oldBitmap, (int) (bm.getWidth()
						* scale + 0.5f), (int) (bm.getHeight() * scale + 0.5f),
						true);
				if (bm != oldBitmap)
					oldBitmap.recycle();
	...
	private static Bitmap nativeDecodeStream(InputStream is, byte[] storage,
		...
		return BitmapFactoryBridge.nativeDecodeStream(is, storage, padding,
				opts);
	}

	private static Bitmap nativeDecodeStream(InputStream is, byte[] storage,
			Rect padding, Options opts, boolean applyScale, float scale) {
		return BitmapFactoryBridge.nativeDecodeStreamScaled(is, storage,
				padding, opts, applyScale, scale);
	}

	private static Bitmap nativeDecodeFileDescriptor(FileDescriptor fd,
			Rect padding, Options opts){
		return BitmapFactoryBridge.nativeDecodeFileDescriptor(fd,padding,opts);
	}

	private static Bitmap nativeDecodeAsset(int asset, Rect padding,
			Options opts){
		return BitmapFactoryBridge.nativeDecodeAsset(asset, padding, opts);
	}

	private static Bitmap nativeDecodeAsset(int asset, Rect padding,
			Options opts, boolean applyScale, float scale){
		return BitmapFactoryBridge.nativeDecodeAsset(asset, padding, opts,applyScale,scale);
	}

	private static Bitmap nativeDecodeByteArray(byte[] data, int offset,
			int length, Options opts){
		return BitmapFactoryBridge.nativeDecodeByteArray(data, offset, length, opts);
	}

	private static byte[] nativeScaleNinePatch(byte[] chunk,
			float scale, Rect pad){
		return BitmapFactoryBridge.nativeScaleNinePatch(chunk, scale, pad);
	}

	private static boolean nativeIsSeekable(FileDescriptor fd){
		return BitmapFactoryBridge.nativeIsSeekable(fd);
	}
}