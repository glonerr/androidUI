...
public class BitmapFactory {
    ...
    public static Bitmap decodeStream(InputStream is, Rect outPadding, Options opts) {
        ...
        if (is instanceof AssetManager.AssetInputStream) {
            ...
            if (opts == null || (opts.inScaled && opts.inBitmap == null)) {
                ...
                bm = nativeDecodeStreamProxy(is, tempStorage, outPadding, opts, true, scale);
                if (bm != null && targetDensity != 0) bm.setDensity(targetDensity);
    ...
    private static Bitmap nativeDecodeStreamProxy(InputStream is, byte[] storage,
            Rect padding, Options opts){
    	Bitmap bitmap = new Bitmap(true);
    	bitmap.mImage = new Image(null, is);
    	return bitmap;
    }
    private static Bitmap nativeDecodeStreamProxy(InputStream is, byte[] storage,
            Rect padding, Options opts, boolean applyScale, float scale){
    ...
    private static Bitmap nativeDecodeFileDescriptorProxy(FileDescriptor fd,
            Rect padding, Options opts){
    	return null;
    }
    private static Bitmap nativeDecodeAssetProxy(int asset, Rect padding, Options opts){
    	return null;
    }
    private static Bitmap nativeDecodeAssetProxy(int asset, Rect padding, Options opts,
            boolean applyScale, float scale){
    ...
    private static Bitmap nativeDecodeByteArrayProxy(byte[] data, int offset,
            int length, Options opts){
    	return null;
    }
    private static byte[] nativeScaleNinePatchProxy(byte[] chunk, float scale, Rect pad){
    	return null;
    }
    private static boolean nativeIsSeekableProxy(FileDescriptor fd){
    	return false;
    }
}
