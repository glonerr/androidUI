...
public final class ViewRootImpl implements ViewParent,
	...
	/**
	 * Set this system property to true to force the view hierarchy to render at
	 * 60 Hz. This can be used to measure the potential framerate.
	 */
	...
	/**
	 * Maximum time we allow the user to roll the trackball enough to generate a
	 * key event, before resetting the counters.
	 */
	...
	/**
	 * Set to true while in performTraversals for detecting when die(true) is
	 * called from internal callbacks such as onMeasure, onPreDraw, onDraw and
	 * deferring doDie() until later.
	 */
	...
	public ViewRootImpl(Context context, Display display) {
		...
		mWindowSession = new BridgeWindowSession();
		// mWindowSession =
		...
		mFallbackEventHandler = PolicyManager
				.makeNewFallbackEventHandler(context);
		mProfileRendering = Boolean.parseBoolean(SystemProperties.get(
				PROPERTY_PROFILE_RENDERING, "false"));
	...
	/**
	 * @return True if the application requests the use of a separate render
				...
				if (!sRenderThreadQueried) {
					...
					try {
						...
						if (applicationInfo.metaData != null) {
							sUseRenderThread = applicationInfo.metaData
									.getBoolean(
											"android.graphics.renderThread",
											false);
						}
	...
	/**
	 * Call this to profile the next traversal call. FIXME for perf testing
	 * only. Remove eventually
	 */
	public void profile() {
		mProfile = true;
	}
		...
		synchronized (this) {
			if (mView == null) {
				...
				if (view instanceof RootViewSurfaceTaker) {
					mSurfaceHolderCallback = ((RootViewSurfaceTaker) view)
				...
				// If the application owns the surface, don't enable hardware
				// acceleration
				...
				mAttachInfo.mApplicationScale = mTranslator == null ? 1.0f
						: mTranslator.applicationScale;
				if (panelParentView != null) {
					mAttachInfo.mPanelParentWindowToken = panelParentView
				...
				if (mTranslator != null) {
					mTranslator
							.translateRectInScreenToAppWindow(mAttachInfo.mContentInsets);
				}
				...
				if (view instanceof RootViewSurfaceTaker) {
					mInputQueueCallback = ((RootViewSurfaceTaker) view)
	...
	public boolean attachFunctor(int functor) {
		...
		if (mAttachInfo.mHardwareRenderer != null
				&& mAttachInfo.mHardwareRenderer.isEnabled()) {
			return mAttachInfo.mHardwareRenderer.attachFunctor(mAttachInfo,
					functor);
		}
		return false;
	}
	...
	private void enableHardwareAcceleration(Context context,
			WindowManager.LayoutParams attrs) {
		mAttachInfo.mHardwareAccelerated = false;
		mAttachInfo.mHardwareAccelerationRequested = false;

		// Don't enable hardware acceleration when the application is in
		// compatibility mode
		if (mTranslator != null)
			return;
		...
		if (hardwareAccelerated) {
			...
			// sSystemRendererDisabled is set,
			// PRIVATE_FLAG_FORCE_HARDWARE_ACCELERATED
			// can be used by code on the system process to escape that and
			// enable
			...
			if (!HardwareRenderer.sRendererDisabled
				...
				// Don't enable hardware acceleration when we're not on the main
				// thread
				if (!HardwareRenderer.sSystemRendererDisabled
						&& Looper.getMainLooper() != Looper.myLooper()) {
					Log.w(HardwareRenderer.LOG_TAG,
				...
				mAttachInfo.mHardwareRenderer = HardwareRenderer
						.createGlRenderer(2, translucent);
				...
				// applications, so they will look more like the app being
				// launched.
	...
	void setLayoutParams(WindowManager.LayoutParams attrs, boolean newView) {
		synchronized (this) {
			...
			// transfer over system UI visibility values as they carry current
			// state.
	...
	void handleScreenStateChange(boolean on) {
		if (on != mAttachInfo.mScreenOn) {
			...
			if (mView != null) {
				mView.dispatchScreenStateChanged(on ? View.SCREEN_STATE_ON
	...
	public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
		...
		final boolean intersected = localDirty.intersect(0, 0, (int) (mWidth
				* appScale + 0.5f), (int) (mHeight * appScale + 0.5f));
		if (!intersected) {
			localDirty.setEmpty();
		}
	 ...
	 * traversal. This list will be cleared after the transitions on the list
	 * are start()'ed. These transitionsa re added by LayoutTransition itself
	 * when it sets up animations. The setup happens during the layout phase of
	 * traversal, which we want to complete before any of the animations are
	 * started (because those animations may side-effect properties that layout
	 * depends upon, like the bounding rectangles of the affected views). So we
	 * add the transition to the list and it is started just prior to starting
	...
	public void requestTransitionStart(LayoutTransition transition) {
		if (mPendingTransitions == null
				|| !mPendingTransitions.contains(transition)) {
			if (mPendingTransitions == null) {
	...
	void unscheduleTraversals() {
		if (mTraversalScheduled) {
			...
			mChoreographer.removeCallbacks(Choreographer.CALLBACK_TRAVERSAL,
					mTraversalRunnable, null);
		}
	}
	...
	private boolean measureHierarchy(final View host,
			final WindowManager.LayoutParams lp, final Resources res,
		...
		if (lp.width == ViewGroup.LayoutParams.WRAP_CONTENT) {
			...
			if (baseSize != 0 && desiredWindowWidth > baseSize) {
				...
				if ((host.getMeasuredWidthAndState() & View.MEASURED_STATE_TOO_SMALL) == 0) {
					...
					if (DEBUG_DIALOG)
						Log.v(TAG, "Window " + mView + ": next baseSize="
								+ baseSize);
	...
	private void performTraversals() {
		...
		if (mFirst) {
			...
			// caches:
			// PixelFormat.hasAlpha(lp.format) || lp.format ==
			// PixelFormat.RGBX_8888
			// However, windows are now always 32 bits by default, so choose 32
			// bits
			...
			// Set the layout direction if it has not been set before (inherit
			// is the default)
		...
		if (layoutRequested) {
			...
			if (mFirst) {
				...
				if (!mPendingVisibleInsets.equals(mAttachInfo.mVisibleInsets)) {
					...
					if (DEBUG_LAYOUT)
						Log.v(TAG, "Visible insets changing to: "
		...
		if (mFitSystemWindowsRequested) {
			...
			if (mLayoutRequested) {
				...
				windowSizeMayChange |= measureHierarchy(host, lp, mView
						.getContext().getResources(), desiredWindowWidth,
						desiredWindowHeight);
			}
		}
		...
		boolean windowShouldResize = layoutRequested
				&& windowSizeMayChange
				&& ((mWidth != host.getMeasuredWidth() || mHeight != host
						.getMeasuredHeight())
		...
		if (mFirst || windowShouldResize || insetsChanged
			...
			try {
				...
				if (DEBUG_LAYOUT)
					Log.v(TAG,
							"relayout: frame=" + frame.toShortString()
									+ " content="
									+ mPendingContentInsets.toShortString()
									+ " visible="
									+ mPendingVisibleInsets.toShortString()
									+ " surface=" + mSurface);

				if (mPendingConfiguration.seq != 0) {
					if (DEBUG_CONFIGURATION)
						Log.v(TAG, "Visible with new config: "
								+ mPendingConfiguration);
					updateConfiguration(mPendingConfiguration, !mFirst);
					mPendingConfiguration.seq = 0;
				}

				contentInsetsChanged = !mPendingContentInsets
						.equals(mAttachInfo.mContentInsets);
				visibleInsetsChanged = !mPendingVisibleInsets
						.equals(mAttachInfo.mVisibleInsets);
				if (contentInsetsChanged) {
					if (mWidth > 0
							...
							&& lp != null
							&& !PixelFormat.formatHasAlpha(lp.format)) {

						disposeResizeBuffer();

						boolean completed = false;
						HardwareCanvas hwRendererCanvas = mAttachInfo.mHardwareRenderer
								.getCanvas();
						HardwareCanvas layerCanvas = null;
						try {
							if (mResizeBuffer == null) {
								mResizeBuffer = mAttachInfo.mHardwareRenderer
										.createHardwareLayer(mWidth, mHeight,
												false);
							...
							Log.w(TAG,
									"Not enough memory for content change anim buffer",
									e);
						} finally {
							if (layerCanvas != null) {
					...
					if (DEBUG_LAYOUT)
						Log.v(TAG, "Content insets changing to: "
				...
				if (!hadSurface) {
					if (mSurface.isValid()) {
						...
						// window manager about all of the windows being
						// displayed
						...
						if (mAttachInfo.mHardwareRenderer != null) {
							try {
								hwInitialized = mAttachInfo.mHardwareRenderer
										.initialize(mHolder.getSurface());
							} catch (Surface.OutOfResourcesException e) {
								Log.e(TAG,
										"OutOfResourcesException initializing HW surface",
										e);
								try {
								...
								mLayoutRequested = true; // ask wm for a new
															// surface next
															// time.
								return;
							}
					...
					try {
						...
						Log.e(TAG,
								"OutOfResourcesException updating HW surface",
								e);
						try {
							if (!mWindowSession.outOfMemory(mWindow)) {
								Slog.w(TAG,
						...
						mLayoutRequested = true; // ask wm for a new surface
													// next time.
						return;
					}
				}
			...
			if (DEBUG_ORIENTATION)
				Log.v(TAG, "Relayout returned: frame=" + frame + ", surface="
						+ mSurface);
			...
			if (mSurfaceHolder != null) {
				...
				if (mSurface.isValid()) {
					if (!hadSurface) {
						...
						SurfaceHolder.Callback callbacks[] = mSurfaceHolder
								.getCallbacks();
						if (callbacks != null) {
			...
			if (!mStopped) {
				...
				if (focusChangedDueToTouchMode
					...
					int childWidthMeasureSpec = getRootMeasureSpec(mWidth,
							lp.width);
					int childHeightMeasureSpec = getRootMeasureSpec(mHeight,
							lp.height);

					if (DEBUG_LAYOUT)
						Log.v(TAG,
								"Ooops, something changed!  mWidth=" + mWidth
										+ " measuredWidth="
										+ host.getMeasuredWidth() + " mHeight="
										+ mHeight + " measuredHeight="
										+ host.getMeasuredHeight()
										+ " coveredInsetsChanged="
										+ contentInsetsChanged);
			...
			// Not the first pass and no window/insets/visibility change but the
			// window
			// may have moved and we need check that and if so to update the
			// left and right
			// in the attach info. We translate only the window frame since on
			// window move
			// the window manager tells us only for the new frame but the insets
			// are the
			// same and we do not want to translate them more than once.

			// TODO: Well, we are checking whether the frame has changed
			// similarly
			// to how this is done for the insets. This is however incorrect
			// since
			// the insets and the frame are translated. For example, the old
			// frame
			// was (1, 1 - 1, 1) and was translated to say (2, 2 - 2, 2), now
			// the new
			// reported frame is (2, 2 - 2, 2) which implies no change but this
			// is not
			// true since we are comparing a not translated value to a
			// translated one.
		...
		if (didLayout) {
			...
			if ((host.mPrivateFlags & View.PFLAG_REQUEST_TRANSPARENT_REGIONS) != 0) {
				...
				if (mTranslator != null) {
					mTranslator
							.translateRegionInWindowToScreen(mTransparentRegion);
				}
		...
		if (computesInternalInsets) {
			...
			if (insetsPending || !mLastGivenInsets.equals(insets)) {
				...
				if (mTranslator != null) {
					contentInsets = mTranslator
							.getTranslatedContentInsets(insets.contentInsets);
					visibleInsets = mTranslator
							.getTranslatedVisibleInsets(insets.visibleInsets);
					touchableRegion = mTranslator
							.getTranslatedTouchableArea(insets.touchableRegion);
				} else {
		...
		if (mFirst) {
			...
			if (mView != null) {
				if (!mView.hasFocus()) {
					...
					if (DEBUG_INPUT_RESIZE)
						Log.v(TAG, "First: requested focused view="
								+ mFocusedView);
				} else {
					mRealFocusedView = mView.findFocus();
					if (DEBUG_INPUT_RESIZE)
						Log.v(TAG, "First: existing focused view="
								+ mRealFocusedView);
				}
			}
		...
		if (!cancelDraw && !newSurface) {
			...
			if (viewVisibility == View.VISIBLE) {
				// Try again
				scheduleTraversals();
			} else if (mPendingTransitions != null
					&& mPendingTransitions.size() > 0) {
				for (int i = 0; i < mPendingTransitions.size(); ++i) {
	...
	private void performLayout() {
		...
		final View host = mView;
		if (DEBUG_ORIENTATION || DEBUG_LAYOUT) {
			Log.v(TAG, "Laying out " + host + " to (" + host.getMeasuredWidth()
					+ ", " + host.getMeasuredHeight() + ")");
		}
				...
				mRenderProfiler = new Thread(new Runnable() {
					...
					public void run() {
						...
						while (mRenderProfilingEnabled) {
							...
							try {
								// TODO: This should use vsync when we get an
								// API
								Thread.sleep(15);
	...
	private void trackFPS() {
		// Tracks frames per second drawn. First value in a series of draws may
		// be bogus
					...
					for (SurfaceHolder.Callback c : callbacks) {
						if (c instanceof SurfaceHolder.Callback2) {
							((SurfaceHolder.Callback2) c)
									.surfaceRedrawNeeded(mSurfaceHolder);
						}
					}
				}
			}
	...
	private void draw(boolean fullRedrawNeeded) {
		...
		int resizeAlpha = 0;
		if (mResizeBuffer != null) {
			long deltaTime = SystemClock.uptimeMillis()
		...
		if (DEBUG_ORIENTATION || DEBUG_DRAW) {
			Log.v(TAG, "Draw " + mView + "/" + mWindowAttributes.getTitle()
					+ ": dirty={" + dirty.left + "," + dirty.top + ","
					+ dirty.right + "," + dirty.bottom + "} surface=" + surface
					+ " surface.isValid()=" + surface.isValid() + ", appScale:"
	...
	private boolean drawSoftware(Surface surface, AttachInfo attachInfo,
			int yoff, boolean scalingRequired, Rect dirty) {

		// If we get here with a disabled & requested hardware renderer,
		// something went
		// wrong (an invalidate posted right before we destroyed the hardware
		// surface
		// for instance) so we should just bail out. Locking the surface with
		// software
		// rendering at this point would lock it forever and prevent hardware
		// renderer
		...
		try {
			...
			try {
				...
				if (!attachInfo.mSetIgnoreDirtyState) {
					// Only clear the flag if it was not set during the
					// mView.draw() call
	 ...
	 * Since adding a style for all possible view is not a viable option we have
	 * this specialized drawing method.
	 * 
	...
	private void drawAccessibilityFocusedDrawableIfNeeded(Canvas canvas) {
		AccessibilityManager manager = AccessibilityManager
				.getInstance(mView.mContext);
		if (!manager.isEnabled() || !manager.isTouchExplorationEnabled()) {
			return;
		}
		if (mAccessibilityFocusedHost == null
				|| mAccessibilityFocusedHost.mAttachInfo == null) {
			return;
		}
		...
		bounds.intersect(0, 0, mAttachInfo.mViewRootImpl.mWidth,
				mAttachInfo.mViewRootImpl.mHeight);
	...
	private Drawable getAccessibilityFocusedDrawable() {
		if (mAttachInfo != null) {
			...
			if (mAttachInfo.mAccessibilityFocusDrawable == null) {
				...
				final boolean resolved = mView.mContext.getTheme()
						.resolveAttribute(R.attr.accessibilityFocusedDrawable,
								value, true);
				if (resolved) {
					mAttachInfo.mAccessibilityFocusDrawable = mView.mContext
							.getResources().getDrawable(value.resourceId);
				}
			}
	...
	boolean scrollToRectOrFocus(Rect rectangle, boolean immediate) {
		...
		if (vi.left > ci.left || vi.top > ci.top || vi.right > ci.right
				|| vi.bottom > ci.bottom) {
			...
			// which may have been removed from the view hierarchy. The
			// following
			...
			if (DEBUG_INPUT_RESIZE)
				Log.v(TAG, "Eval scroll: focus=" + focus + " rectangle="
						+ rectangle + " ci=" + ci + " vi=" + vi);
			if (focus == mLastScrolledFocus && !mScrollMayChange
				...
				if (focus.getGlobalVisibleRect(mVisRect, null)) {
					...
					if (rectangle == null) {
						...
						if (DEBUG_INPUT_RESIZE)
							Log.v(TAG, "Focus in window: focusRect="
									+ mTempRect.toShortString() + " visRect="
		...
		if (scrollY != mScrollY) {
			if (DEBUG_INPUT_RESIZE)
				Log.v(TAG, "Pan scroll changed: old=" + mScrollY + " , new="
						+ scrollY);
	...
	void setAccessibilityFocus(View view, AccessibilityNodeInfo node) {
		...
		if (mAccessibilityFocusedVirtualView != null) {
			...
			AccessibilityNodeProvider provider = focusHost
					.getAccessibilityNodeProvider();
			if (provider != null) {
	...
	public void requestChildFocus(View child, View focused) {
		...
		mAttachInfo.mTreeObserver.dispatchOnGlobalFocusChange(mOldFocusedView,
				focused);
		scheduleTraversals();
	...
	public void clearChildFocus(View child) {
		...
		if (focusSearch(null, View.FOCUS_FORWARD) == null) {
			mAttachInfo.mTreeObserver.dispatchOnGlobalFocusChange(
	...
	public void focusableViewAvailable(View v) {
		...
		if (mView != null) {
			if (!mView.hasFocus()) {
				...
				// the one case where will transfer focus away from the current
				// one
				// is if the current view is a view group that prefers to give
				// focus
				// to its children first AND the view is a descendant of it.
				mFocusedView = mView.findFocus();
				boolean descendantsHaveDibsOnFocus = (mFocusedView instanceof ViewGroup)
						&& (((ViewGroup) mFocusedView)
								.getDescendantFocusability() == ViewGroup.FOCUS_AFTER_DESCENDANTS);
				if (descendantsHaveDibsOnFocus
						&& isViewDescendantOf(v, mFocusedView)) {
					// If a view gets the focus, the listener will be invoked
					// from requestChildFocus()
					v.requestFocus();
				}
			}
		}
	...
	void dispatchDetachedFromWindow() {
		...
		mAccessibilityManager
				.removeAccessibilityStateChangeListener(mAccessibilityInteractionConnectionManager);
		...
		// Dispose the input channel after removing the window so the Window
		// Manager
		// doesn't interpret the input channel being closed as an abnormal
		// termination.
		if (mInputChannel != null) {
	...
	void updateConfiguration(Configuration config, boolean force) {
		if (DEBUG_CONFIGURATION)
			Log.v(TAG,
					"Applying new config to window "
							+ mWindowAttributes.getTitle() + ": " + config);
		...
		if (mView != null) {
			...
			if (force || mLastConfiguration.diff(config) != 0) {
				final int lastLayoutDirection = mLastConfiguration
						.getLayoutDirection();
	...
	final class ViewRootHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_INVALIDATE_RECT:
				...
				info.target.invalidate(info.left, info.top, info.right,
						info.bottom);
				info.release();
				break;
			case MSG_IME_FINISHED_EVENT:
			...
			case MSG_WINDOW_FOCUS_CHANGED: {
				if (mAdded) {
					...
					if (hasWindowFocus) {
						...
						if (mAttachInfo.mHardwareRenderer != null
							...
							try {
								...
								// Retry in a bit.
								sendMessageDelayed(
										obtainMessage(msg.what, msg.arg1,
												msg.arg2), 500);
								return;
							}
						}
					...
					if (hasWindowFocus) {
						...
						// Clear the forward bit. We can just do this directly,
						// since
			...
			case MSG_DISPATCH_KEY_FROM_IME: {
				if (LOCAL_LOGV)
					Log.v(TAG, "Dispatching key " + msg.obj + " from IME to "
							+ mView);
				KeyEvent event = (KeyEvent) msg.obj;
				...
				enqueueInputEvent(event, null,
						QueuedInputEvent.FLAG_DELIVER_POST_IME, true);
			}
				break;
			case MSG_FINISH_INPUT_CONNECTION: {
				InputMethodManager imm = InputMethodManager.peekInstance();
			...
			case MSG_DISPATCH_DRAG_LOCATION_EVENT: {
				...
				event.mLocalState = mLocalDragState; // only present when this
														// app called
														// startDrag()
	...
	/**
	 * Something in the current window tells us we need to change the touch
	 * mode. For example, we are not in touch mode, and the user touches the
	 * screen.
	 * 
	...
	private boolean ensureTouchModeLocally(boolean inTouchMode) {
		if (DBG)
			Log.d("touchmode", "ensureTouchModeLocally(" + inTouchMode
					+ "), current " + "touch mode is "
			...
			if (mView.hasFocus()) {
				...
				if (focused != null && !focused.isFocusableInTouchMode()) {
					...
					if (ancestorToTakeFocus != null) {
						...
						// nothing appropriate to have focus in touch mode,
						// clear it out
						mView.unFocus();
						mAttachInfo.mTreeObserver.dispatchOnGlobalFocusChange(
								focused, null);
		...
		if (mView != null) {
			if (mView.hasFocus()) {
				...
				if (!(mFocusedView instanceof ViewGroup)) {
					...
					return false;
				} else if (((ViewGroup) mFocusedView)
						.getDescendantFocusability() != ViewGroup.FOCUS_AFTER_DESCENDANTS) {
					// some view group has focus, and doesn't prefer its
					// children
			...
			// find the best view to give focus to in this brave new
			// non-touch-mode
			// world
	...
	private void deliverPointerEvent(QueuedInputEvent q) {
		...
		if (MEASURE_LATENCY) {
			lt.sample("A Dispatching PointerEvents",
		...
		if (MEASURE_LATENCY) {
			lt.sample("B Dispatched PointerEvents ",
					System.nanoTime() - event.getEventTimeNano());
		}
		if (handled) {
		...
		if (mView != null && mAdded
			...
			if (mLastWasImTarget) {
				...
				if (imm != null) {
					...
					if (DEBUG_IMF)
						Log.v(TAG, "Sending trackball event to IME: seq=" + seq
								+ " event=" + event);
	...
	private void deliverTrackballEventPostIme(QueuedInputEvent q) {
		...
		// Translate the trackball event into DPAD keys and try to deliver
		// those.
		...
		case MotionEvent.ACTION_DOWN:
			...
			enqueueInputEvent(new KeyEvent(curTime, curTime,
					...
					KeyEvent.FLAG_FALLBACK, InputDevice.SOURCE_KEYBOARD));
			break;
		...
		if (DEBUG_TRACKBALL)
			Log.v(TAG,
					"TB X=" + x.position + " step=" + x.step + " dir=" + x.dir
							+ " acc=" + x.acceleration + " move="
							+ event.getX() + " / Y=" + y.position + " step="
							+ y.step + " dir=" + y.dir + " acc="
							+ y.acceleration + " move=" + event.getY());
		...
		if (keycode != 0) {
			...
			if (DEBUG_TRACKBALL)
				Log.v(TAG, "Move: movement=" + movement + " accelMovement="
						+ accelMovement + " accel=" + accel);
			if (accelMovement > movement) {
				...
				enqueueInputEvent(new KeyEvent(curTime, curTime,
						KeyEvent.ACTION_MULTIPLE, keycode, repeatCount,
						metaState, KeyCharacterMap.VIRTUAL_KEYBOARD, 0,
						KeyEvent.FLAG_FALLBACK, InputDevice.SOURCE_KEYBOARD));
			}
			while (movement > 0) {
		...
		// Unfortunately we can't tell whether the application consumed the
		// keys, so
	...
	private void deliverGenericMotionEvent(QueuedInputEvent q) {
		...
		if (mView != null && mAdded
				&& (q.mFlags & QueuedInputEvent.FLAG_DELIVER_POST_IME) == 0) {
			if (LOCAL_LOGV)
				Log.v(TAG, "Dispatching generic motion " + event + " to "
						+ mView);
				...
				if (imm != null) {
					...
					if (DEBUG_IMF)
						Log.v(TAG, "Sending generic motion event to IME: seq="
								+ seq + " event=" + event);
					imm.dispatchGenericMotionEvent(mView.getContext(), seq,
	...
	private void updateJoystickDirection(MotionEvent event,
		...
		int xDirection = joystickAxisValueToDirection(event
				.getAxisValue(MotionEvent.AXIS_HAT_X));
		if (xDirection == 0) {
			xDirection = joystickAxisValueToDirection(event.getX());
		}

		int yDirection = joystickAxisValueToDirection(event
		...
		if (xDirection != mLastJoystickXDirection) {
			if (mLastJoystickXKeyCode != 0) {
				enqueueInputEvent(new KeyEvent(time, time, KeyEvent.ACTION_UP,
						mLastJoystickXKeyCode, 0, metaState, deviceId, 0,
						KeyEvent.FLAG_FALLBACK, source));
			...
			if (xDirection != 0 && synthesizeNewKeys) {
				mLastJoystickXKeyCode = xDirection > 0 ? KeyEvent.KEYCODE_DPAD_RIGHT
						: KeyEvent.KEYCODE_DPAD_LEFT;
				enqueueInputEvent(new KeyEvent(time, time,
						KeyEvent.ACTION_DOWN, mLastJoystickXKeyCode, 0,
						metaState, deviceId, 0, KeyEvent.FLAG_FALLBACK, source));
			}
		}
		...
		if (yDirection != mLastJoystickYDirection) {
			if (mLastJoystickYKeyCode != 0) {
				enqueueInputEvent(new KeyEvent(time, time, KeyEvent.ACTION_UP,
						mLastJoystickYKeyCode, 0, metaState, deviceId, 0,
						KeyEvent.FLAG_FALLBACK, source));
			...
			if (yDirection != 0 && synthesizeNewKeys) {
				mLastJoystickYKeyCode = yDirection > 0 ? KeyEvent.KEYCODE_DPAD_DOWN
						: KeyEvent.KEYCODE_DPAD_UP;
				enqueueInputEvent(new KeyEvent(time, time,
						KeyEvent.ACTION_DOWN, mLastJoystickYKeyCode, 0,
						metaState, deviceId, 0, KeyEvent.FLAG_FALLBACK, source));
			}
		}
	}
	...
	private boolean checkForLeavingTouchModeAndConsume(KeyEvent event) {
		...
		// Only consider leaving touch mode on DOWN or MULTIPLE actions, never
		// on UP.
		final int action = event.getAction();
		...
		if (mView != null && mAdded
			...
			if (mLastWasImTarget) {
				...
				if (imm != null) {
					final int seq = event.getSequenceNumber();
					if (DEBUG_IMF)
						Log.v(TAG, "Sending key event to IME: seq=" + seq
								+ " event=" + event);
	...
	void handleImeFinishedEvent(int seq, boolean handled) {
		...
		if (q != null && q.mEvent.getSequenceNumber() == seq) {
			if (DEBUG_IMF) {
				Log.v(TAG, "IME finished event: seq=" + seq + " handled="
						+ handled + " event=" + q);
			}
			if (handled) {
				finishInputEvent(q, true);
				...
				if (q.mEvent instanceof KeyEvent) {
					...
					if (event.getAction() != KeyEvent.ACTION_UP) {
						// If the window doesn't currently have input focus,
						// then drop
						// this event. This could be an event that came back
						// from the
						// IME dispatch but the window has lost focus in the
						// meantime.
						if (!mAttachInfo.mHasWindowFocus) {
							Slog.w(TAG,
	...
	private void deliverKeyEventPostIme(QueuedInputEvent q) {
		...
		// If the key's purpose is to exit touch mode then we consume it and
		// consider it handled.
		...
		// If the Control modifier is held, try to interpret the key as a
		// shortcut.
		...
		if (event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			if (direction != 0) {
				...
				if (focused != null) {
					...
					// Give the focused view a last chance to handle the dpad
					// key.
	...
	private void handleDragEvent(DragEvent event) {
		// From the root, only drag start/end/location are dispatched.
		// entered/exited
		// are determined and dispatched by the viewgroup hierarchy, who then
		// report
		...
		if (mView != null && mAdded) {
			...
			if (what == DragEvent.ACTION_DRAG_EXITED) {
				// A direct EXITED event means that the window manager knows
				// we've just crossed
				// a window boundary, so the current drag target within this one
				// must have
				// just been exited. Send it the usual notifications and then
				// we're done
				// for now.
				mView.dispatchDragEvent(event);
			} else {
				// Cache the drag description when the operation starts, then
				// fill it in
				// on subsequent calls as a convenience
				if (what == DragEvent.ACTION_DRAG_STARTED) {
					mCurrentDragView = null; // Start the current-recipient
												// tracking
				...
				// For events with a [screen] location, translate into window
				// coordinates
				if ((what == DragEvent.ACTION_DRAG_LOCATION)
						|| (what == DragEvent.ACTION_DROP)) {
					mDragPoint.set(event.mX, event.mY);
					if (mTranslator != null) {
						mTranslator
								.translatePointInScreenToAppWindow(mDragPoint);
					}
	...
	public void handleDispatchSystemUiVisibilityChanged(
		...
		if (mSeq != args.seq) {
			...
			// sure to do a traversal afterward so the window manager is given
			// our
			// most recent data.
			mSeq = args.seq;
		...
		if (args.localChanges != 0) {
			mView.updateLocalSystemUiVisibility(args.localValue,
					args.localChanges);
		}
		if (mAttachInfo != null) {
			int visibility = args.globalVisibility
	...
	private AudioManager getAudioManager() {
		if (mView == null) {
			throw new IllegalStateException(
					"getAudioManager called when there is no mView");
		}
	...
	private int relayoutWindow(WindowManager.LayoutParams params,
		...
		int relayoutResult = mWindowSession
				.relayout(
						mWindow,
						mSeq,
						params,
						...
						insetsPending ? WindowManagerGlobal.RELAYOUT_INSETS_PENDING
								: 0, mWinFrame, mPendingContentInsets,
	...
	public void playSoundEffect(int effectId) {
		...
		try {
			...
			case SoundEffectConstants.NAVIGATION_DOWN:
				audioManager
						.playSoundEffect(AudioManager.FX_FOCUS_NAVIGATION_DOWN);
				return;
			case SoundEffectConstants.NAVIGATION_LEFT:
				audioManager
						.playSoundEffect(AudioManager.FX_FOCUS_NAVIGATION_LEFT);
				return;
			case SoundEffectConstants.NAVIGATION_RIGHT:
				audioManager
						.playSoundEffect(AudioManager.FX_FOCUS_NAVIGATION_RIGHT);
				return;
			...
			default:
				throw new IllegalArgumentException("unknown effect id "
						+ effectId + " not defined in "
						+ SoundEffectConstants.class.getCanonicalName());
			}
		} catch (IllegalStateException e) {
			// Exception thrown by getAudioManager() when mView is null
			Log.e(TAG, "FATAL EXCEPTION when attempting to play sound effect: "
					+ e);
			e.printStackTrace();
		}
	}
	...
	public boolean performHapticFeedback(int effectId, boolean always) {
		try {
			return mWindowSession.performHapticFeedback(mWindow, effectId,
					always);
		} catch (RemoteException e) {
			return false;
		}
	...
	public View focusSearch(View focused, int direction) {
		...
		return FocusFinder.getInstance().findNextFocus((ViewGroup) mView,
				focused, direction);
	}
	...
	public void die(boolean immediate) {
		// Make sure we do execute immediately if we are in the middle of a
		// traversal or the damage
			...
			if (mAdded && !mFirst) {
				...
				if (mView != null) {
					...
					if (mWindowAttributesChanged || viewVisibilityChanged) {
						...
						// animation info.
						try {
							if ((relayoutWindow(mWindowAttributes,
									viewVisibility, false) & WindowManagerGlobal.RELAYOUT_RES_FIRST_TIME) != 0) {
								mWindowSession.finishDrawing(mWindow);
							}
						} catch (RemoteException e) {
						}
	...
	public void dispatchFinishInputConnection(InputConnection connection) {
		Message msg = mHandler.obtainMessage(MSG_FINISH_INPUT_CONNECTION,
				connection);
		mHandler.sendMessage(msg);
	}
	...
	public void dispatchResized(Rect frame, Rect contentInsets,
		...
		if (DEBUG_LAYOUT)
			Log.v(TAG, "Resizing " + this + ": frame=" + frame.toShortString()
					+ " contentInsets=" + contentInsets.toShortString()
					+ " visibleInsets=" + visibleInsets.toShortString()
					+ " reportDraw=" + reportDraw);
		Message msg = mHandler.obtainMessage(reportDraw ? MSG_RESIZED_REPORT
				: MSG_RESIZED);
		if (mTranslator != null) {
		...
		final boolean sameProcessCall = (Binder.getCallingPid() == android.os.Process
				.myPid());
		args.arg1 = sameProcessCall ? new Rect(frame) : frame;
		args.arg2 = sameProcessCall ? new Rect(contentInsets) : contentInsets;
		args.arg3 = sameProcessCall ? new Rect(visibleInsets) : visibleInsets;
		args.arg4 = sameProcessCall && newConfig != null ? new Configuration(
				newConfig) : newConfig;
		msg.obj = args;
		mHandler.sendMessage(msg);
	}
	...
	public void dispatchMoved(int newX, int newY) {
		if (DEBUG_LAYOUT)
			Log.v(TAG, "Window moved " + this + ": newX=" + newX + " newY="
					+ newY);
		if (mTranslator != null) {
	 ...
	 * 
	 * However, because the application or IME can synthesize and inject
	 * multiple key events at a time without going through the input dispatcher,
	 * we end up needing a queue on the application's side.
	 */
	...
	void enqueueInputEvent(InputEvent event, InputEventReceiver receiver,
		...
		// Always enqueue the input event in order, regardless of its time
		// stamp.
		// We do this because the application or the IME may inject key events
		// in response to touch events and we want to ensure that the injected
		// keys
		// are processed in the order they were received and we cannot trust
		// that
	...
	final class InvalidateOnAnimationRunnable implements Runnable {
			...
			synchronized (this) {
				...
				if (mPosted && mViews.isEmpty() && mViewRects.isEmpty()) {
					mChoreographer.removeCallbacks(
							Choreographer.CALLBACK_ANIMATION, this, null);
					mPosted = false;
				}
			}
		}
		...
		public void run() {
			...
			synchronized (this) {
				...
				if (viewCount != 0) {
					mTempViews = mViews.toArray(mTempViews != null ? mTempViews
							: new View[viewCount]);
					mViews.clear();
				}
				...
				if (viewRectCount != 0) {
					mTempViewRects = mViewRects
							.toArray(mTempViewRects != null ? mTempViewRects
	...
	public void cancelInvalidate(View view) {
		...
		// fixme: might leak the AttachInfo.InvalidateInfo objects instead of
		// returning
		// them to the pool
	...
	public void dispatchUnhandledKey(KeyEvent event) {
		if ((event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0) {
			...
			KeyCharacterMap.FallbackAction fallbackAction = kcm
					.getFallbackAction(keyCode, metaState);
			if (fallbackAction != null) {
				final int flags = event.getFlags() | KeyEvent.FLAG_FALLBACK;
				KeyEvent fallbackEvent = KeyEvent.obtain(event.getDownTime(),
						event.getEventTime(), event.getAction(),
						fallbackAction.keyCode, event.getRepeatCount(),
						fallbackAction.metaState, event.getDeviceId(),
	...
	public void dispatchSystemUiVisibilityChanged(int seq,
			int globalVisibility, int localValue, int localChanges) {
		...
		mHandler.sendMessage(mHandler.obtainMessage(
				MSG_DISPATCH_SYSTEM_UI_VISIBILITY, args));
	}
	 ...
	 * {@link AccessibilityEvent#TYPE_WINDOW_CONTENT_CHANGED} event. This event
	 * is send at most once every
	...
	private void postSendWindowContentChangedCallback(View source) {
		...
		if (oldSource == null) {
			...
			mSendWindowContentChangedAccessibilityEvent.mSource = getCommonPredecessor(
					oldSource, source);
		}
	}
	...
	public boolean requestSendAccessibilityEvent(View child,
		...
		case AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUSED: {
			...
			final int accessibilityViewId = AccessibilityNodeInfo
					.getAccessibilityViewId(sourceNodeId);
			View source = mView.findViewByAccessibilityId(accessibilityViewId);
			if (source != null) {
				AccessibilityNodeProvider provider = source
						.getAccessibilityNodeProvider();
				if (provider != null) {
					AccessibilityNodeInfo node = provider
							.createAccessibilityNodeInfo(AccessibilityNodeInfo
									.getVirtualDescendantId(sourceNodeId));
	...
	class TakenSurfaceHolder extends BaseSurfaceHolder {
		...
		public void onRelayoutContainer() {
			// Not currently interesting -- from changing between fixed and
			// layout size.
		}
	...
	static final class InputMethodCallback implements
			InputMethodManager.FinishedEventCallback {
	...
	static class W extends IWindow.Stub {
		...
		private static int checkCallingPermission(String permission) {
			try {
				return ActivityManagerNative.getDefault().checkPermission(
						permission, Binder.getCallingPid(),
				...
				if (view != null) {
					if (checkCallingPermission(Manifest.permission.DUMP) != PackageManager.PERMISSION_GRANTED) {
						throw new SecurityException(
								"Insufficient permissions to invoke"
					...
					try {
						clientStream = new ParcelFileDescriptor.AutoCloseOutputStream(
								out);
						ViewDebug.dispatchCommand(view, command, parameters,
								clientStream);
					} catch (IOException e) {
						e.printStackTrace();
					} finally {
		...
		public void dispatchWallpaperOffsets(float x, float y, float xStep,
				float yStep, boolean sync) {
			if (sync) {
				try {
		...
		public void dispatchSystemUiVisibilityChanged(int seq,
			...
			final ViewRootImpl viewAncestor = mViewAncestor.get();
			if (viewAncestor != null) {
				viewAncestor.dispatchSystemUiVisibilityChanged(seq,
						globalVisibility, localValue, localChanges);
			}
		}
	...
	static final class TrackballAxis {
		 ...
		 * for us to consider the user to be doing fast trackball movements, and
		 * thus apply an acceleration.
		 */
		static final long FAST_MOVE_TIME = 150;

		/**
		 * Scaling factor to the time (in milliseconds) between events to how
		 * much to multiple/divide the current acceleration. When movement is <
		 * FAST_MOVE_TIME this multiplies the acceleration; when >
		 ...
		 * has been reversed, the state is reset before adding the movement (so
		 * that you don't have to compensate for any previously collected
		 * movement before see the result of the movement in the new direction).
		 * 
		...
		float collect(float off, long time, String axis) {
			...
			if (normTime > 0) {
				...
				if (delta < normTime) {
					...
					float scale = (normTime - delta)
							* ACCEL_MOVE_SCALING_FACTOR;
					if (scale > 1)
						acc *= scale;
					if (DEBUG_TRACKBALL)
						Log.v(TAG, axis + " accelerate: off=" + off
								+ " normTime=" + normTime + " delta=" + delta
								+ " scale=" + scale + " acc=" + acc);
					acceleration = acc < MAX_ACCELERATION ? acc
							: MAX_ACCELERATION;
				} else {
					// The user is scrolling slowly, so decrease acceleration.
					float scale = (delta - normTime)
							* ACCEL_MOVE_SCALING_FACTOR;
					if (scale > 1)
						acc /= scale;
					if (DEBUG_TRACKBALL)
						Log.v(TAG, axis + " deccelerate: off=" + off
								+ " normTime=" + normTime + " delta=" + delta
								+ " scale=" + scale + " acc=" + acc);
		...
		/**
		 * Generate the number of discrete movement events appropriate for the
		 * currently collected trackball movement.
		 * 
		 * @param precision
		 *            The minimum movement required to generate the first
		 *            discrete movement.
		 * 
		 * @return Returns the number of discrete movements, either positive or
		 *         negative, or 0 if there is not enough trackball movement yet
	...
	public static final class CalledFromWrongThreadException extends
	 ...
	 */
	final class AccessibilityInteractionConnectionManager implements
		...
		public void ensureConnection() {
			if (mAttachInfo != null) {
				...
				if (!registered) {
					mAttachInfo.mAccessibilityWindowId = mAccessibilityManager
							.addAccessibilityInteractionConnection(mWindow,
									new AccessibilityInteractionConnection(
											ViewRootImpl.this));
				}
			}
		}
		...
		public void ensureNoConnection() {
			...
			if (registered) {
				...
				mAccessibilityManager
						.removeAccessibilityInteractionConnection(mWindow);
			}
		}
	}
	...
	static final class AccessibilityInteractionConnection extends
		...
		@Override
		public void performAccessibilityAction(long accessibilityNodeId,
			...
			if (viewRootImpl != null && viewRootImpl.mView != null) {
				viewRootImpl.getAccessibilityInteractionController()
						.performAccessibilityActionClientThread(
								...
								interactionId, callback, flags,
								interogatingPid, interrogatingTid);
			} else {
				// We cannot make the call and notify the caller so it does not
				// wait.
				try {
					callback.setPerformAccessibilityActionResult(false,
							interactionId);
				} catch (RemoteException re) {
					/* best effort - ignore */
				}
		...
		@Override
		public void findAccessibilityNodeInfoByViewId(long accessibilityNodeId,
				int viewId, int interactionId,
				IAccessibilityInteractionConnectionCallback callback,
				int flags, int interrogatingPid, long interrogatingTid) {
			ViewRootImpl viewRootImpl = mViewRootImpl.get();
			if (viewRootImpl != null && viewRootImpl.mView != null) {
				viewRootImpl.getAccessibilityInteractionController()
						.findAccessibilityNodeInfoByViewIdClientThread(
								accessibilityNodeId, viewId, interactionId,
								callback, flags, interrogatingPid,
								interrogatingTid);
			} else {
				// We cannot make the call and notify the caller so it does not
				// wait.
				try {
					callback.setFindAccessibilityNodeInfoResult(null,
							interactionId);
				} catch (RemoteException re) {
					/* best effort - ignore */
				}
		...
		@Override
		public void findAccessibilityNodeInfosByText(long accessibilityNodeId,
			...
			if (viewRootImpl != null && viewRootImpl.mView != null) {
				viewRootImpl.getAccessibilityInteractionController()
						.findAccessibilityNodeInfosByTextClientThread(
								accessibilityNodeId, text, interactionId,
								callback, flags, interrogatingPid,
								interrogatingTid);
			} else {
				// We cannot make the call and notify the caller so it does not
		...
		@Override
		public void findFocus(long accessibilityNodeId, int focusType,
				int interactionId,
				IAccessibilityInteractionConnectionCallback callback,
				int flags, int interrogatingPid, long interrogatingTid) {
			ViewRootImpl viewRootImpl = mViewRootImpl.get();
			if (viewRootImpl != null && viewRootImpl.mView != null) {
				viewRootImpl.getAccessibilityInteractionController()
						.findFocusClientThread(accessibilityNodeId, focusType,
								interactionId, callback, flags,
		...
		@Override
		public void focusSearch(long accessibilityNodeId, int direction,
				int interactionId,
				IAccessibilityInteractionConnectionCallback callback,
				int flags, int interrogatingPid, long interrogatingTid) {
			ViewRootImpl viewRootImpl = mViewRootImpl.get();
			if (viewRootImpl != null && viewRootImpl.mView != null) {
				viewRootImpl.getAccessibilityInteractionController()
						.focusSearchClientThread(accessibilityNodeId,
								direction, interactionId, callback, flags,
								interrogatingPid, interrogatingTid);
			} else {
				// We cannot make the call and notify the caller so it does not
				// wait.
	...
	private class SendWindowContentChangedAccessibilityEvent implements
			Runnable {
		public View mSource;
