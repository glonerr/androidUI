...
import com.lonerr.bridge.util.StringBlockBridge;
...
final class StringBlock {
			...
			if (style != null) {
				...
				for (int styleIndex = 0; styleIndex < style.length; styleIndex += 3) {
					...
					if (styleId == mStyleIDs.boldId || styleId == mStyleIDs.italicId
							...
							|| styleId == mStyleIDs.bigId || styleId == mStyleIDs.smallId || styleId == mStyleIDs.subId
							|| styleId == mStyleIDs.supId || styleId == mStyleIDs.strikeId
							|| styleId == mStyleIDs.listItemId || styleId == mStyleIDs.marqueeId) {
	...
	private CharSequence applyStyles(String str, int[] style, StyleIDs ids) {
		...
		while (i < style.length) {
			int type = style[i];
			if (localLOGV)
				Log.v(TAG, "Applying style span id=" + type + ", start=" + style[i + 1] + ", end=" + style[i + 2]);

			if (type == ids.boldId) {
				buffer.setSpan(new StyleSpan(Typeface.BOLD), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (type == ids.italicId) {
				buffer.setSpan(new StyleSpan(Typeface.ITALIC), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				...
				buffer.setSpan(new TypefaceSpan("monospace"), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (type == ids.bigId) {
				buffer.setSpan(new RelativeSizeSpan(1.25f), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (type == ids.smallId) {
				buffer.setSpan(new RelativeSizeSpan(0.8f), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				...
				buffer.setSpan(new SuperscriptSpan(), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (type == ids.strikeId) {
				buffer.setSpan(new StrikethroughSpan(), style[i + 1], style[i + 2] + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (type == ids.listItemId) {
				addParagraphSpan(buffer, new BulletSpan(10), style[i + 1], style[i + 2] + 1);
			} else if (type == ids.marqueeId) {
				buffer.setSpan(TextUtils.TruncateAt.MARQUEE, style[i + 1], style[i + 2] + 1,
				...
				if (tag.startsWith("font;")) {
					...
					if (sub != null) {
						int size = Integer.parseInt(sub);
						buffer.setSpan(new AbsoluteSizeSpan(size, true), style[i + 1], style[i + 2] + 1,
								Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
					}
					...
					if (sub != null) {
						...
						buffer.setSpan(new ForegroundColorSpan(color), style[i + 1], style[i + 2] + 1,
								Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
					}
					...
					if (sub != null) {
						int color = XmlUtils.convertValueToUnsignedInt(sub, -1);
						buffer.setSpan(new BackgroundColorSpan(color), style[i + 1], style[i + 2] + 1,
								Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
					}
					...
					sub = subtag(tag, ";href=");
					if (sub != null) {
						buffer.setSpan(new URLSpan(sub), style[i + 1], style[i + 2] + 1,
								Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
					}
					...
					for (int t = tag.indexOf(';'); t < len; t = next) {
						...
						buffer.setSpan(new Annotation(key, value), style[i + 1], style[i + 2] + 1,
								Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
					}
				}
			}
	...
	/**
	 * If a translator has messed up the edges of paragraph-level markup, fix it
	 * to actually cover the entire paragraph that it is attached to instead of
	 * just whatever range they put it on.
	 */
	...
	/**
	 * Forces the text line to be the specified height, shrinking/stretching the
	 * ascent if possible, or the descent if shrinking the ascent further will
	...
	/**
	 * Create from an existing string block native object. This is -extremely-
	...
	private static final int nativeCreate(byte[] data, int offset, int size) {
		return StringBlockBridge.nativeCreate(data, offset, size);
	}

	private static final int nativeGetSize(int obj) {
		return StringBlockBridge.nativeGetSize(obj);
	}

	private static final String nativeGetString(int obj, int idx) {
		return StringBlockBridge.nativeGetString(obj, idx);
	}

	private static final int[] nativeGetStyle(int obj, int idx) {
		return StringBlockBridge.nativeGetStyle(obj, idx);
	}

	private static final void nativeDestroy(int obj){
		StringBlockBridge.nativeDestroy(obj);
	}
}