...
public final class XmlBlock {
	...
	/* package */final class Parser implements XmlResourceParser {
		...
		public String getAttributeValue(String namespace, String name) {
			...
			if (idx >= 0) {
				...
				if (DEBUG)
					System.out.println("Namespace=" + getAttributeNamespace(idx) + "Name=" + getAttributeName(idx)
	...
	private static final int nativeCreate(byte[] data, int offset, int size) {
		return XmlBlockBridge.nativeCreate(data, offset, size);
	}

	private static final int nativeGetStringBlock(int obj) {
		return XmlBlockBridge.nativeGetStringBlock(obj);
	}

	private static final int nativeCreateParseState(int obj) {
		return XmlBlockBridge.nativeCreateParseState(obj);
	}

	/* package */static final int nativeNext(int state) throws XmlPullParserException {
		return XmlBlockBridge.nativeNext(state);
	}

	private static final int nativeGetNamespace(int state) {
		return XmlBlockBridge.nativeGetNamespace(state);
	}

	/* package */static final int nativeGetName(int state) {
		return XmlBlockBridge.nativeGetName(state);
	}

	private static final int nativeGetText(int state) {
		return XmlBlockBridge.nativeGetText(state);
	}

	private static final int nativeGetLineNumber(int state) {
		return XmlBlockBridge.nativeGetLineNumber(state);
	}

	private static final int nativeGetAttributeCount(int state) {
		return XmlBlockBridge.nativeGetAttributeCount(state);
	}

	private static final int nativeGetAttributeNamespace(int state, int idx) {
		return XmlBlockBridge.nativeGetAttributeNamespace(state, idx);
	}

	private static final int nativeGetAttributeName(int state, int idx){
		return XmlBlockBridge.nativeGetAttributeName(state, idx);
	}

	private static final int nativeGetAttributeResource(int state, int idx){
		return XmlBlockBridge.nativeGetAttributeResource(state, idx);
	}

	private static final int nativeGetAttributeDataType(int state, int idx){
		return XmlBlockBridge.nativeGetAttributeDataType(state, idx);
	}

	private static final int nativeGetAttributeData(int state, int idx){
		return XmlBlockBridge.nativeGetAttributeData(state, idx);
	}

	private static final int nativeGetAttributeStringValue(int state, int idx){
		return XmlBlockBridge.nativeGetAttributeStringValue(state, idx);
	}

	private static final int nativeGetIdAttribute(int state){
		return XmlBlockBridge.nativeGetIdAttribute(state);
	}

	private static final int nativeGetClassAttribute(int state){
		return XmlBlockBridge.nativeGetClassAttribute(state);
	}

	private static final int nativeGetStyleAttribute(int state){
		return XmlBlockBridge.nativeGetStyleAttribute(state);
	}

	private static final int nativeGetAttributeIndex(int state, String namespace, String name){
		return XmlBlockBridge.nativeGetAttributeIndex(state,namespace, name);
	}

	private static final void nativeDestroyParseState(int state){
		XmlBlockBridge.nativeDestroyParseState(state);
	}

	private static final void nativeDestroy(int obj){
		XmlBlockBridge.nativeDestroy(obj);
	}
}